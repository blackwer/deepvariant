...
function run_call_variants() {
  ...
  python ./bazel-out/k8-opt/bin/deepvariant/call_variants.zip \
...
function run_postprocess_variants() {
  ...
  python ./bazel-out/k8-opt/bin/deepvariant/postprocess_variants.zip \
      --ref "${REF}" \
...
function run_vcf_stats_report() {
  ...
  python ./bazel-out/k8-opt/bin/deepvariant/vcf_stats_report.zip \
